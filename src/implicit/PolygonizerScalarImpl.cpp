...
void CMPUProcessor::process_cells_continuation(const FieldComputer& fc, MPU& mpu) const
{
	...
	GENERIC_POLYMORPHIC_STACK<vec3i, ctCells> stkCells;
	GENERIC_POLYMORPHIC_STACK<vec3f, ctCells> stkSeeds;
	...
	//Find a seed point
	vec3f lo = mpu.bboxLo;
	vec3f hi = vadd3f(lo, vec3f(mpuSide, mpuSide, mpuSide));
	vec3f seed;
	...
	//Shoot Rays
	{
	#ifdef SIMD_USE_M128
		//My SIMD VARS
		vec3f c = vadd3f(lo, vec3f(halfSide, halfSide, halfSide));
		...
		//Shoot to MPU FACE-MIDS [6]
		{
			...
			if(stkSeeds.empty())
			{
				...
				{
					seed = vec3f(fc.m_blobPrims.seedX[i], fc.m_blobPrims.seedY[i], fc.m_blobPrims.seedZ[i]);
	...
	while(!stkSeeds.empty())
	{
		...
		{
			//Fetch and Mark
			vec3i idxCell = stkCells.top();
			vec3f cellLo = vadd3f(mpu.bboxLo, vec3f(idxCell.x * m_cellsize, idxCell.y * m_cellsize, idxCell.z * m_cellsize));
			...
			for(int icase=0; icase<16; icase++)
			{
				...
				if(candidate != -1)
				{
					...
					if(idxMeshVertex[icase] == -1)
					{
						...
						vec3f e1 = vadd3f(mpu.bboxLo, vec3f(m_cellsize * sx, m_cellsize * sy, m_cellsize * sz));
						vec3f e2 = e1;
					...
					for(int icase=0; icase<16; icase++)
					{
						...
						if(candidate != -1)
						{
							...
							if(idxMeshVertex[icase] == -1)
							{
								...
								idxMissingEdge[ctMissing] = vec3i(idxEdgeStart, idxEdgeEnd, icase);
								ctMissing++;
							}
							ctEdges++;
						}
					...
					for(int iEdgeBlk = 0; iEdgeBlk < szEdgeBlocks; iEdgeBlk++)
					{
						...
						while(idxSIMDLine < PS_SIMD_FLEN)
						{
							if(idxEdge < ctMissing)
							{
								...
								vec3f e1 = vadd3f(mpu.bboxLo, vec3f(m_cellsize * cellCornerIDX[idxEdgeStart].x, m_cellsize * cellCornerIDX[idxEdgeStart].y, m_cellsize * cellCornerIDX[idxEdgeStart].z));
								vec3f e2 = vadd3f(mpu.bboxLo, vec3f(m_cellsize * cellCornerIDX[idxEdgeEnd].x, m_cellsize * cellCornerIDX[idxEdgeEnd].y, m_cellsize * cellCornerIDX[idxEdgeEnd].z));
								//vec3f p = vadd3f(e1, vscale3f(scale, vsub3f(e2, e1)));
								vec3f p;
								float fp;
			...
			for(int k=0; k<GRID_DIM-1; k++)
			{
				...
				for(int icase=0; icase<16; icase++)
				{
					...
					if(idxMeshVertex[icase] == -1)
					{
						...
						e1 = vec3f(posX[interval - 1], posY[interval - 1], posZ[interval - 1]);
						e2 = vec3f(posX[interval], posY[interval], posZ[interval]);
				...
				if((idxCellConfig != 0)&&(idxCellConfig != 255))
				{
					...
					for(int icase=0; icase<16; icase++)
					{
						...
						if(candidate != -1)
						{
							...
							{
								float scale = (ISO_VALUE - cellCornerField[idxEdgeStart])/(cellCornerField[idxEdgeEnd] - cellCornerField[idxEdgeStart]);
								vec3f e1 = vec3f::add(mpu.bboxLo, vec3f(m_cellsize * cellCornerIDX[idxEdgeStart].x, m_cellsize * cellCornerIDX[idxEdgeStart].y, m_cellsize * cellCornerIDX[idxEdgeStart].z));
								vec3f e2 = vec3f::add(mpu.bboxLo, vec3f(m_cellsize * cellCornerIDX[idxEdgeEnd].x, m_cellsize * cellCornerIDX[idxEdgeEnd].y, m_cellsize * cellCornerIDX[idxEdgeEnd].z));
