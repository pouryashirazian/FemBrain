...
int FastRBF::init() {
	...
	int prg = m_lpGPU->addProgramFromFile(strPolyFP.cptr());
	assert(prg >= 0);
...
//	m_lpGPU->getKernelWorkgroupSize(m_lpKernelComputeElements);
	...
	m_lpKernelApplyDeformations = m_lpGPU->addKernel(prg, "ApplyVertexDeformations");
	m_lpGPU->getKernelWorkgroupSize(m_lpKernelApplyDeformations);

	m_lpKernelComputeFieldArray = m_lpGPU->addKernel(prg, "ComputeRBFPrimFieldArray");
...
//	m_lpGPU->getKernelWorkgroupSize(m_lpKernelComputeOffSurfacePointsAndFields);
	...
	if(errNum != CL_SUCCESS)
	{
		LogErrorArg1("Failed to create VertexCountTable! Ocl Error: %s", oclErrorString(errNum));
	...
	if(errNum != CL_SUCCESS)
	{
		LogErrorArg1("Failed to create TriableTable! Ocl Error: %s", oclErrorString(errNum));
		return ERR_GPUPOLY_TRITABLE_NOT_READ;
	}
#endif
...
bool FastRBF::copyVertexBufferToRestPos() {
	...
	cl_mem inMemMeshVertex = m_lpGPU->createMemBufferFromGL(m_vboVertex, PS::CL::memReadOnly);
...
bool FastRBF::setupFromMesh(Mesh* lpMesh) {
	...
	//Setup Mesh
	this->m_aabb = lpNode->computeBoundingBox();
	this->cleanup();
