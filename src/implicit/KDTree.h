...
struct PS_BEGIN_ALIGNED(PS_SIMD_FLEN) SOALinearKDTree
{
	...
	void initLeaf(U32 idx, U32* primNums, int np, const vec3f& lo, const vec3f& hi)
	{
	    flags[idx] = 3;
	    nPrims[idx] |= (np << 2);
	...
	void initInterior(U32 idx, U32 axis, U32 ac, float s, const vec3f& lo, const vec3f& hi)
	{
		split[idx] = s;
		flags[idx] = axis;
		aboveChild[idx] |= (ac << 2);
...
private:
	void buildTree(int nodeNum,
				   const vec3f& nodeBoundsLo,
				   const vec3f& nodeBoundsHi,
