...
void GenerateMassMatrix::computeMassMatrix(VolumetricMesh * volumetricMesh,
		SparseMatrix ** massMatrix, bool inflate3Dim) {
		...
		for (int el = 0; el < volumetricMesh->getNumElements(); el++) {
			...
			for (int i = 0; i < numElementVertices; i++)
				for (int j = 0; j < numElementVertices; j++) {
					...
					massMatrixOutline->AddEntry(3 * indexi + 0, 3 * indexj + 0,
							entry);
					massMatrixOutline->AddEntry(3 * indexi + 1, 3 * indexj + 1,
							entry);
					massMatrixOutline->AddEntry(3 * indexi + 2, 3 * indexj + 2,
							entry);
				}
		}
	}
...
void GenerateMassMatrix::computeVertexMasses(VolumetricMesh * volumetricMesh,
		double * masses) {
