...
ObjMesh::ObjMesh(const std::string & filename_, int verbose) {
	...
	addMaterial("default", Vec3d(0.2, 0.2, 0.2), Vec3d(0.6, 0.6, 0.6),
			Vec3d(0.0, 0.0, 0.0), 65);
	...
	while (ifs) {
		...
		if (strncmp(line, "v ", 2) == 0) // vertex
			...
			if (sscanf(line, "vt %lf %lf\n", &x, &y) < 2) {
				throw ObjMeshException("Invalid texture coordinate", filename,
						lineNum);
			}
			tex = Vec3d(x, y, 0);
			...
			char s[4096];
			if ((sscanf(line, "g %s\n", s) < 1) && verbose)
				cout << "Warning:  Empty group name encountered: " << filename
						<< " " << lineNum << endl;
			std::string name = s;
			...
			for (std::vector<Group>::const_iterator itr = groups.begin();
					itr != groups.end(); itr++) {
				if (itr->getName() == name) {
			...
			while (*curPos != '\0') {
				...
				if (strstr(curPos, "//") != NULL) {
					...
					if (sscanf(curPos, "%u//%u", &pos, &nor) < 2) {
						throw ObjMeshException("Invalid face", filename,
								lineNum);
					}
			...
			numGroupFaces++;
				...
				|| (strncmp(line, "\0", 1) == 0)) { // ignore comment lines and empty lines
			...
			if (numGroupFaces > 0) {
				...
				char newNameC[4096];
				sprintf(newNameC, "%s.%d", groupSourceName.c_str(),
						groupCloneIndex);
			...
			if (ignoreCounter < 5) {
				if (verbose)
					std::cout << "Warning: ignoring '" << command << "' line"
							<< std::endl;
				ignoreCounter++;
			}
			if (ignoreCounter == 5) {
				if (verbose)
					std::cout << "(suppressing further output of ignored lines)"
							<< std::endl;
				ignoreCounter++;
			}
		} else {
	...
	// statistics
	if (verbose) {
		std::cout << "Parsed obj file '" << filename << "'; statistics:"
				<< std::endl;
		std::cout << "   " << groups.size() << " groups," << std::endl;
		std::cout << "   " << numFaces << " faces," << std::endl;
		std::cout << "   " << vertexPositions.size() << " vertices,"
				<< std::endl;
		std::cout << "   " << normals.size() << " normals, " << std::endl;
		std::cout << "   " << textureCoordinates.size()
				<< " texture coordinates, " << std::endl;
	}
}

ObjMesh::ObjMesh(int numVertices, double * vertices, int numTriangles,
		int * triangles) {
	for (int i = 0; i < numVertices; i++)
		addVertexPosition(
				Vec3d(vertices[3 * i + 0], vertices[3 * i + 1],
						vertices[3 * i + 2]));
...
std::vector<std::string> ObjMesh::getGroupNames() const {
	...
	for (std::vector<Group>::const_iterator groupItr = groups.begin();
...
void ObjMesh::printInfo() const {
	...
	for (SVec::const_iterator nameItr = groupNames1.begin();
			nameItr != groupNames1.end(); nameItr++) {
		...
		for (unsigned int iFace = 0; iFace < group1.getNumFaces(); iFace++) {
			...
			for (unsigned int iVertex = 0; iVertex < face.getNumVertices();
					iVertex++) {
				if (iVertex != 0)
					std::cout << " -> ";
			...
			for (unsigned int iVertex = 0; iVertex < face.getNumVertices();
				...
				std::cout << "      position = "
						<< getPosition(vertex.getPositionIndex()) << ";"
						<< std::endl;
				if (vertex.hasNormalIndex())
					std::cout << "      normal = "
							<< getNormal(vertex.getNormalIndex()) << ";"
							<< std::endl;
				if (vertex.hasTextureCoordinateIndex())
					std::cout << "      texture coordinate = "
							<< getTextureCoordinate(
									vertex.getTextureCoordinateIndex()) << ";"
							<< std::endl;
			}
		}
	}
...
void ObjMesh::triangulate() {
	for (unsigned int i = 0; i < groups.size(); i++) // over all groups
		for (unsigned int j = 0; j < groups[i].getNumFaces(); j++) // over all faces
			...
			if (face->getNumVertices() < 3) {
				printf(
						"Warning: encountered a face with fewer than 3 vertices.\n");
			}
...
void ObjMesh::getBoundingBox(double expansionRatio, Vec3d * bmin_,
		Vec3d * bmax_) const {
	*bmin_ = center - expansionRatio * cubeHalf;
	*bmax_ = center + expansionRatio * cubeHalf;
}

void ObjMesh::getCubicBoundingBox(double expansionRatio, Vec3d * bmin_,
		Vec3d * bmax_) const {
...
void ObjMesh::save(const string & filename, int outputMaterials) const {
	...
	if (outputMaterials) {
		...
		for (unsigned int i = 0; i < strlen(materialFilenameTempC); i++)
			if ((materialFilenameTempC[i] == '/')
					|| (materialFilenameTempC[i] == '\\'))
	...
	fout << "# Number of texture coordinates: " << textureCoordinates.size()
			<< endl;
	...
	for (unsigned int i = 0; i < normals.size(); i++) {
		...
		fout << "vn " << normal_[0] << " " << normal_[1] << " " << normal_[2]
				<< endl;
	}
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		...
		if (outputMaterials)
			fout << "usemtl "
					<< materials[groups[i].getMaterialIndex()].getName()
					<< endl;
		...
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			if (face.getNumVertices() < 3)
				cout << "Warning: encountered a face (group=" << i << ",face="
						<< iFace << ") with fewer than 3 vertices." << endl;
	...
	if (outputMaterials) {
		...
		if (!fout) {
			cout << "Error: could not write to file " << materialFilename
					<< endl;
			return;
		}
...
void ObjMesh::saveToAbq(const string & filename) const {
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		...
		for (unsigned int numFaceVertices = 3; numFaceVertices <= 4;
				numFaceVertices++) {
			bool firstElement = true;
			for (unsigned int iFace = 0; iFace < groups[i].getNumFaces();
					iFace++) {
				Face face = groups[i].getFace(iFace); // get face whose number is iFace
...
void ObjMesh::saveToStl(const string & filename) const {
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			fout << "      vertex " << p0[0] << " " << p0[1] << " " << p0[2]
					<< endl;
			fout << "      vertex " << p1[0] << " " << p1[1] << " " << p1[2]
					<< endl;
			fout << "      vertex " << p2[0] << " " << p2[1] << " " << p2[2]
					<< endl;
...
ObjMesh * ObjMesh::splitIntoConnectedComponents() const {
	...
	for (unsigned int i = 0; i < groups.size(); i++) // over all groups
		for (unsigned int j = 0; j < groups[i].getNumFaces(); j++) // over all faces
			...
			if (j % 100 == 1)
				printf(
						"Processing group %d / %d, face %d / %d . Current num groups: %d\n",
						i, (int) groups.size(), j,
						(int) groups[i].getNumFaces(), (int) components.size());
			const Face * face = groups[i].getFaceHandle(j);
			if (face->getNumVertices() < 3) {
				printf(
						"Warning: encountered a face with fewer than 3 vertices.\n");
			...
			for (int vtx = 0; vtx < faceDegree; vtx++) {
				...
				int vertexFound = 0;
				for (componentType::iterator iter = components.begin();
			...
			if (belongingComponents.size() == 0) {
				...
				map<int, set<int> >::iterator zeroIter = components.find(
						firstComponent);

				// add free vertices into the first component
				for (vector<int>::iterator iter = freeVertices.begin();
						iter != freeVertices.end(); iter++)
				...
				if (belongingComponents.size() >= 2) {
					...
					set<int>::iterator secondElement =
							belongingComponents.begin();
					secondElement++;
					for (set<int>::iterator setIter = secondElement;
							setIter != belongingComponents.end(); setIter++) {
						// merge into 0
						map<int, set<int> >::iterator componentIter =
								components.find(*setIter);
						for (set<int>::iterator iter =
								componentIter->second.begin();
								iter != componentIter->second.end(); iter++)
							zeroIter->second.insert(*iter);
					}
	...
	for (componentType::iterator componentIter = components.begin();
			componentIter != components.end(); componentIter++) {
		for (set<int>::iterator setIter = componentIter->second.begin();
				setIter != componentIter->second.end(); setIter++) {
			int vertex = *setIter;
			map<int, int>::iterator vertexIterator = vertexGroup.find(vertex);
			if (vertexIterator != vertexGroup.end()) {
				printf(
						"Error: encountered a vertex in more than one group (internal error).\n");
			}
			vertexGroup.insert(
					make_pair(vertex, sortedID[componentIter->first]));
		}
	}
		...
		for (unsigned int j = 0; j < groups[i].getNumFaces(); j++) // over all faces
			...
			for (int vtx = 0; vtx < faceDegree; vtx++) {
				...
				if (vertexGroup[vertex] != groupID) {
					printf(
							"Error: encountered a face belonging to more than one group (internal error).\n");
				}
			}
...
ObjMesh * ObjMesh::clone(
		const std::vector<std::pair<int, int> > & groupsAndFaces,
		int removeIsolatedVertices) const {
	ObjMesh * output = new ObjMesh();
	...
	for (std::vector<std::pair<int, int> >::const_iterator iter =
			groupsAndFaces.begin(); iter != groupsAndFaces.end(); iter++)
		output->groups[iter->first].addFace(
				groups[iter->first].getFace(iter->second));
...
ObjMesh * ObjMesh::extractGroup(unsigned int groupID, int keepOnlyUsedNormals,
		int keepOnlyUsedTextureCoordinates) const {
	...
	for (unsigned int j = 0; j < groups[groupID].getNumFaces(); j++) // over all faces
		...
		for (int vtx = 0; vtx < faceDegree; vtx++) {
			...
			if (face->getVertex(vtx).hasNormalIndex()) {
				...
				if (oldToNewNormals.find(normalIndex)
						== oldToNewNormals.end()) {
					oldToNewNormals.insert(
							make_pair(normalIndex, numGroupNormals));
					newToOldNormals.insert(
							make_pair(numGroupNormals, normalIndex));
					numGroupNormals++;
				}
			}
			...
			if (face->getVertex(vtx).hasTextureCoordinateIndex()) {
				int textureCoordinateIndex =
						face->getVertex(vtx).getTextureCoordinateIndex();
				if (oldToNewTextureCoordinates.find(textureCoordinateIndex)
						== oldToNewTextureCoordinates.end()) {
					oldToNewTextureCoordinates.insert(
							make_pair(textureCoordinateIndex,
									numGroupTextureCoordinates));
					newToOldTextureCoordinates.insert(
							make_pair(numGroupTextureCoordinates,
	...
	if (keepOnlyUsedTextureCoordinates) {
		for (int i = 0; i < numGroupTextureCoordinates; i++)
			output->addTextureCoordinate(
					getTextureCoordinate(newToOldTextureCoordinates[i]));
	} else {
		for (unsigned int i = 0; i < getNumTextureCoordinates(); i++)
			output->addTextureCoordinate(getTextureCoordinate(i));
	}
	...
	for (unsigned int j = 0; j < groups[groupID].getNumFaces(); j++) // over all faces
		...
		for (int vtx = 0; vtx < faceDegree; vtx++) {
			...
			if (newVertex.hasTextureCoordinateIndex()
					&& keepOnlyUsedTextureCoordinates) {
				int oldTextureCoordinateIndex =
						face->getVertex(vtx).getTextureCoordinateIndex();
				int newTextureCoordinateIndex =
						oldToNewTextureCoordinates[oldTextureCoordinateIndex];
...
void ObjMesh::interpolateToCentroids(const std::vector<double> & nodalData,
		std::vector<double> & centroidData) const {
	int faceIndex = 0;
	// over all faces
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
...
void ObjMesh::interpolateToCentroids(const std::vector<Vec3d> & nodalData,
		std::vector<Vec3d> & centroidData) const {
	int faceIndex = 0;
	// over all faces
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			for (unsigned int iVertex = 0; iVertex < face.getNumVertices();
					iVertex++) {
				unsigned int index = face.getVertex(iVertex).getPositionIndex();
				surfaceAreaPerVertex[index] += faceSurfaceArea
						/ face.getNumVertices(); // each vertex owns an equal share of the face
			}
		}
	}
}
...
void ObjMesh::transformRigidly(const Mat3d & rotation,
...
double ObjMesh::computeVolume() const {
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			if (face.getNumVertices() != 3)
				cout << "Warning: encountered a face (group=" << i << ",face="
						<< iFace << ") which is not a triangle." << endl;
...
Vec3d ObjMesh::computeCenterOfMass_Triangles(
		const vector<double> & groupDensities) const {
...
double ObjMesh::computeMass(const vector<double> & groupDensities) const {
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		...
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			if (face.getNumVertices() < 3) {
				printf(
						"Warning: encountered a face with fewer than three vertices.\n");
				continue;
			}
...
void ObjMesh::computeInertiaTensor_Triangles(
		const vector<double> & groupDensities, double IT[6]) const {
	Vec3d centerOfMass = 0.0;
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		...
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			Vec3d v0 = getPosition(face.getVertex(0));
			for (unsigned int iVertex = 1; iVertex < face.getNumVertices() - 1;
					iVertex++) {
	...
	double correction[6] = { b * b + c * c, -a * b, -a * c, a * a + c * c, -b
			* c, a * a + b * b };
...
void ObjMesh::computeMassPerVertex(
		const vector<double> & groupSurfaceMassDensity,
		vector<double> & masses) const {
	masses.clear();
	for (unsigned int i = 0; i < getNumVertices(); i++)
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		...
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			for (unsigned int iVertex = 0; iVertex < face.getNumVertices();
					iVertex++)
				masses[face.getVertex(iVertex).getPositionIndex()] +=
						groupSurfaceMassDensity[i] * faceSurfaceArea
								/ face.getNumVertices();
		}
	}
}
...
void ObjMesh::setNormalsToAverageFaceNormals() {
	...
	for (unsigned int i = 0; i < getNumVertices(); i++) {
		if (normalCount[i] == 0) {
			if (!errorMessageSeen)
				cout
						<< "Warning: encountered a vertex not belonging to any triangle (suppressing further warnings)"
						<< endl;
...
unsigned int ObjMesh::findClosestVertex(const Vec3d & queryPos,
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			for (unsigned k = 0; k < face.getNumVertices(); k++) {
				...
				Vec3d pos1 = getPosition(
						face.getVertex((k + 1) % face.getNumVertices()));
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			for (unsigned k = 0; k < face.getNumVertices(); k++) {
				...
				*vtxb =
						face.getVertex((k + 1) % face.getNumVertices()).getPositionIndex();
			}
		}
	}
...
void ObjMesh::buildVertexNormals(double angle) {
	if (vertexFaceNeighbors.size() == 0) {
		printf(
				"Error: buildNornals() failed because vertex face neighbors were not built prior to calling buildNormals(). Call \"buildVertexFaceNeighbors\".\n");
		return;
	}
	...
	for (unsigned int i = 0; i < getNumVertices(); i++) {
		...
		const Face * firstFace = getGroupHandle(
				vertexFaceNeighbors[i].begin()->getGroupIndex())->getFaceHandle(
				vertexFaceNeighbors[i].begin()->getFaceIndex());
		...
		for (std::list<VertexFaceNeighbor>::iterator iter =
				vertexFaceNeighbors[i].begin();
				iter != vertexFaceNeighbors[i].end(); iter++) {
			//get angle
			const Face * currentFace =
					getGroupHandle(iter->getGroupIndex())->getFaceHandle(
							iter->getFaceIndex());
		...
		for (std::list<VertexFaceNeighbor>::iterator iter =
			...
			if (iter->getAveraged()) {
				...
				Vertex * vertex = (Vertex*) currentFace->getVertexHandle(
						iter->getFaceVertexIndex());
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			for (unsigned int j = 0; j < faceHandle->getNumVertices(); j++) {
				...
				int vertexIndex =
						faceHandle->getVertexHandle(j)->getPositionIndex();
				Vec3d newNorm(0.0);
				bool averagedAnything = false;

				for (std::list<VertexFaceNeighbor>::iterator iter =
						vertexFaceNeighbors[vertexIndex].begin();
						iter != vertexFaceNeighbors[vertexIndex].end();
						iter++) {
					const Face * neighborFaceHandle = getGroupHandle(
							iter->getGroupIndex())->getFaceHandle(
							iter->getFaceIndex());
					if (!neighborFaceHandle->hasFaceNormal())
						printf("Warning: face has no face normal\n");

					if (dot(faceNorm, neighborFaceHandle->getFaceNormal())
							> cosang) {
...
void ObjMesh::computePseudoNormals() {
		...
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			for (unsigned k = 0; k < face.getNumVertices(); k++) {
				...
				if (k != 0)
					...
					posPrev = getPosition(
							face.getVertex(face.getNumVertices() - 1));

				double lenNext = len(posNext - pos);
				double lenPrev = len(posPrev - pos);

				double angle = acos(
						dot(posNext - pos, posPrev - pos) / lenNext / lenPrev);
				Vec3d normal = norm(cross(posNext - pos, posPrev - pos));

				if (isNaN(normal[0]) || isNaN(normal[1]) || isNaN(normal[2])) {
					cout
							<< "Error (when computing vertex pseudonormals): NaN encountered (face with zero surface area)."
							<< endl;
					...
					if ((lenNext == 0) || (lenPrev == 0) || isNaN(angle)) {
						cout
								<< "Warning (when computing vertex pseudonormals): encountered zero-length edge"
								<< endl;
						cout << "  lenNext: " << lenNext << " lenPrev: "
								<< lenPrev << " angle: " << angle << endl;
					} else {
						pseudoNormals[face.getVertex(k).getPositionIndex()] +=
								angle * normal;
	...
	for (unsigned int i = 0; i < getNumVertices(); i++) {
		if (vertexDegree[i] != 0) {
			...
			if (isNaN(pseudoNormals[i][0]) || isNaN(pseudoNormals[i][1])
					|| isNaN(pseudoNormals[i][2])) {
				cout
						<< "Error (when computing vertex pseudonormals): NaN encountered."
						<< endl;
				cout << "Vertex: " << i << " pseudoNormal="
						<< pseudoNormals[i][0] << " " << pseudoNormals[i][1]
						<< " " << pseudoNormals[i][2] << endl;
				cout << "  Pseudonormal before normalization=";
				printf("%G %G %G\n", pseudoNormalRaw[0], pseudoNormalRaw[1],
						pseudoNormalRaw[2]);
...
void ObjMesh::computeEdgePseudoNormals() {
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			if (isNaN(normal[0]) || isNaN(normal[1]) || isNaN(normal[2])) {
				cout << "Error: nan encountered (face with zero surface area)."
						<< endl;
				cout << "Group: " << i << " Face: " << iFace << " " << endl;
				exit(1);
			}
			...
			for (unsigned k = 0; k < face.getNumVertices(); k++) {
				...
				unsigned int endVertex = face.getVertex(
						(k + 1) % face.getNumVertices()).getPositionIndex();
	...
	// normalize normals
	map<pair<unsigned int, unsigned int>, Vec3d>::iterator iter;
	for (iter = edgePseudoNormals.begin(); iter != edgePseudoNormals.end();
			++iter) {
		Vec3d normal = norm(iter->second);
		if (isNaN(normal[0]) || isNaN(normal[1]) || isNaN(normal[2])) {
			cout
					<< "Warning (while computing edge pseudonormals): NaN encountered (face with zero surface area)."
					<< endl;
			normal[0] = 1;
			normal[1] = 0;
			normal[2] = 0;
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			for (unsigned int jj = 0; jj < face.getNumVertices(); jj++)
				for (unsigned int kk = jj + 1; kk < face.getNumVertices();
						kk++) {
					if (face.getVertex(jj).getPositionIndex()
							== face.getVertex(kk).getPositionIndex())
...
int ObjMesh::getEdgePseudoNormal(unsigned int i, unsigned int j,
		Vec3d * pseudoNormal) const {
	pair<unsigned int, unsigned int> edge;
	...
	map<pair<unsigned int, unsigned int>, Vec3d>::const_iterator iter =
			edgePseudoNormals.find(edge);
...
double ObjMesh::computeFaceSurfaceArea(const Face & face) const {
	...
	for (unsigned int iVertex = 1; iVertex < face.getNumVertices() - 1;
		...
		faceSurfaceArea += fabs(
				computeTriangleSurfaceArea(basePos, pos1, pos2));
	}
...
void ObjMesh::computeSpecificInertiaTensor(Vec3d & v0, Vec3d & v1, Vec3d & v2,
		double t[6]) const {
	...
	t[4] = (-2 * v1[1] * v1[2] - v1[2] * v2[1] - v0[2] * (v1[1] + v2[1])
			- v1[1] * v2[2] - 2 * v2[1] * v2[2]
...
void ObjMesh::parseMaterials(const char * objMeshFilename,
	...
	if (!file) {
		fprintf(stderr,
				" parseMaterials() failed: can't open material file %s.\n",
				filename);
	...
	while (fscanf(file, "%s", buf) != EOF) {
		...
		case 'n':
			...
			if (numMaterials >= 1) // flush previous material
				addMaterial(matName, Vec3d(Ka[0], Ka[1], Ka[2]),
						Vec3d(Kd[0], Kd[1], Kd[2]), Vec3d(Ks[0], Ks[1], Ks[2]),
						shininess, textureFile);
		...
		case 'N':
			if (buf[1] == 's') {
				if (fscanf(file, "%lf", &shininess) < 1)
					printf(
							"Warning: bad file syntax. Unable to read shininess.\n");
...
void ObjMesh::initSurfaceSampling() {
	if (!isTriangularMesh()) {
		throw ObjMeshException(
				"Error in init surface sampling: surface not triangular.");
	}
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			surfaceSamplingAreas.push_back(
					make_pair(area, groups[i].getFaceHandle(iFace)));
...
Vec3d ObjMesh::getSurfaceSamplePosition(double sample) const {
	...
	for (facePosition = 0; facePosition < surfaceSamplingAreas.size() - 1;
			facePosition++) {
...
void ObjMesh::getMeshRadius(const Vec3d & centroid, double * radius) const {
	...
	for (std::vector<Vec3d>::const_iterator iter = vertexPositions.begin();
			iter != vertexPositions.end(); iter++) {
...
void ObjMesh::getMeshGeometricParameters(Vec3d * centroid,
											  double * radius) const {
	//find the centroid
...
void ObjMesh::buildVertexFaceNeighbors() {
	...
	for (unsigned int i = 0; i < groups.size(); i++) {
		for (unsigned int iFace = 0; iFace < groups[i].getNumFaces(); iFace++) {
			...
			for (unsigned int j = 0; j < faceHandle->getNumVertices(); j++) {
				const ObjMesh::Vertex * vertexHandle =
						faceHandle->getVertexHandle(j);
				vertexFaceNeighbors[vertexHandle->getPositionIndex()].push_back(
						ObjMesh::VertexFaceNeighbor(i, iFace, j));
			}
		}
	}
}
...
ObjMeshException::ObjMeshException(const std::string & reason,
	...
	std::ostringstream oss;
	oss << "Error in file '" << filename << "', line " << line << ": "
			<< reason;
	std::cout << std::endl << oss.str() << std::endl;
}
...
int ObjMesh::removeDuplicatedMaterials() {
	...
	for (unsigned int i = 0; i < getNumGroups(); i++)
		groups[i].setMaterialIndex(
				reNumberVector[groups[i].getMaterialIndex()]);

	return materials.size();
}

void ObjMesh::exportGeometry(int * numVertices, double ** vertices,
		int * numTriangles, int ** triangles, int * numGroups,
		int ** triangleGroups) const {
	// set vertices
...
void ObjMesh::exportFaceGeometry(int * numVertices, double ** vertices,
		int * numFaces, int ** faceCardinality, int ** faces) const {
	// set vertices
	*numVertices = vertexPositions.size();
	*vertices = (double*) malloc(sizeof(double) * 3 * *numVertices);
	for (int i = 0; i < *numVertices; i++) {
	...
	*faces = (int*) malloc(sizeof(int) * totalCardinality);
	...
	printf(
			"Exported %d vertices and %d faces. Average number of vertices: %G\n",
...
void ObjMesh::mergeGroups(const vector<int> & groupIndicesIn) {
	...
	int count = 0;
	for (vector<int>::reverse_iterator iter = groupIndices.rbegin();
			iter != groupIndices.rend(); iter++) {
...
void ObjMesh::removeEmptyGroups() {
	for (vector<Group>::iterator iter = groups.begin(); iter != groups.end();
			iter++) {
