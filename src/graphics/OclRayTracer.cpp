...
void RayTracer::setup(int w, int h)
{
	...
	strFP += AnsiStr("PS_Shaders/Raytracer.cl");
	...
	int prg = m_lpDevice->addProgramFromFile(strFP.cptr());
	assert(prg >= 0);

	//Build Kernel
	LogInfo("3.Setup Kernel Functions.");
	m_lpKernelPrimary = m_lpDevice->addKernel(prg, "RayTrace");
...
bool RayTracer::run(){
	...
	if (err) {
		LogErrorArg1("Error: Failed to execute RayTrace kernel! (%s)", PS::CL::oclErrorString(err));
		return false;
	}
	...
	if (err) {
		LogErrorArg1("Error: Failed to readback RayTraced image! (%s)", PS::CL::oclErrorString(err));
		SAFE_DELETE_ARRAY(lpImageData);
		return false;
	}

	//Save as png
	AnsiStr strFP = ExtractFilePath(GetExePath()) + "raytracer_output.png";
