...
namespace PS{
	namespace CL {
		...
		void Scan::init(ComputeDevice* lpDevice)
		{
			...
			strFP += AnsiStr("PS_Shaders/Scan.cl");

			m_lpDevice = lpDevice;
            int prgID = m_lpDevice->addProgramFromFile(strFP, true);
            assert(prgID >= 0);

			m_lpKernelScanExclusiveLocal1 = m_lpDevice->addKernel(prgID, "scanExclusiveLocal1");
			m_lpKernelScanExclusiveLocal2 = m_lpDevice->addKernel(prgID, "scanExclusiveLocal2");
			m_lpKernelUniformUpdate = m_lpDevice->addKernel(prgID, "uniformUpdate");
			...
			m_dstBuffer = m_lpDevice->createMemBuffer((MAX_BATCH_ELEMENTS / (4 * WORKGROUP_SIZE)) * sizeof(U32), PS::CL::memReadWrite);
		}
