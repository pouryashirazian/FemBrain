...
public:
	...
	void init();
	bool allocate(const svec3i& workDim);
	void cleanup();

	int setLowerVertex(const svec3f& start, float mpuSide);

	//U32 memUsage() const

	svec3i getWorkDim() const {return m_workDim;}
...
private:
	...
	int prepareOpBBox(int idxOp, svec3f& boxLo, svec3f& boxHi);
