...
void KDTreeAccel::buildTree(int nodeNum, const svec3f& nodeBoundsLo, const svec3f& nodeBoundsHi,
							SOALinearKDTree& kdtree,
							...
							BoundEdge *edges[3], U32* prims0, U32* prims1, int badRefines)
{
	...
	float totalSA = vsurfaceArea3f(nodeBoundsHi, nodeBoundsLo);
	float invTotalSA = 1.f / totalSA;

	svec3f d = vsub3f(nodeBoundsHi, nodeBoundsLo);

	// Choose which axis to split along
	U32 axis = vmaxExtent3f(nodeBoundsHi, nodeBoundsLo);
	int retries = 0;
	retrySplit:
	...
	for (int i = 0; i < nPrimitives; ++i)
	{
		...
		svec3f bboxLo = svec3f(m_lpPrimitives->bboxLoX[pn],
							 m_lpPrimitives->bboxLoY[pn],
							 m_lpPrimitives->bboxLoZ[pn]);

		svec3f bboxHi = svec3f(m_lpPrimitives->bboxHiX[pn],
						     m_lpPrimitives->bboxHiY[pn],
							 m_lpPrimitives->bboxHiZ[pn]);

		edges[axis][2 * i] 	   = BoundEdge(velement3f(bboxLo, axis), pn, true);
		edges[axis][2 * i + 1] = BoundEdge(velement3f(bboxHi, axis), pn, false);
	}
	...
	for (int i = 0; i < 2 * nPrimitives; ++i)
	{
		...
		if (edget > velement3f(nodeBoundsLo, axis) && edget < velement3f(nodeBoundsHi, axis))
		{
			// Compute cost for split at _i_th edge
			U32 otherAxis0 = (axis + 1) % 3;
			U32 otherAxis1 = (axis + 2) % 3;

			float belowSA = 2 * (velement3f(d, otherAxis0) * velement3f(d, otherAxis1) +
								(edget - velement3f(nodeBoundsLo, axis)) * (velement3f(d, otherAxis0) + velement3f(d, otherAxis1)));

			float aboveSA = 2 * (velement3f(d, otherAxis0) * velement3f(d, otherAxis1) +
								(velement3f(nodeBoundsHi, axis) - edget) * (velement3f(d, otherAxis0) + velement3f(d, otherAxis1)));
	...
	svec3f bounds0Lo = nodeBoundsLo;
	svec3f bounds0Hi = nodeBoundsHi;
	svec3f bounds1Lo = nodeBoundsLo;
	svec3f bounds1Hi = nodeBoundsHi;

	vsetElement3f(bounds0Hi, bestAxis, tsplit);
	vsetElement3f(bounds1Lo, bestAxis, tsplit);
