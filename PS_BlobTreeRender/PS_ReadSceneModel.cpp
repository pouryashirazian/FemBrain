...
{
	if(!FileExists(DAnsiStr(lpFilePath)))
	...
	//FileVersion
	CSketchConfig* lpScript = new CSketchConfig(DAnsiStr(lpFilePath), CAppConfig::fmRead);
...
//Recursive read
int ModelReader::readNode(CSketchConfig* lpScript, int id, int* lpOutIsOp)
{
	...
	if(isOp)
	{
		...
		//Add Op
		DAnsiStr strOpType = lpScript->readString(strNodeName, "OperatorType");
	...
	else
	{
		...
		//Set SkeletonType
		DAnsiStr strPrimType = lpScript->readString(strNodeName, "PrimitiveType");
...
int ModelReader::readTransformation(PS::CSketchConfig* lpScript, int id)
{
	DAnsiStr strNodeName = printToAStr("BLOBNODE %d", id);
	...
	//Translate to origin
	mat4Translate(vscale3f(-1.0f, t), matForward);

	//Set Scale
	mat4Scale(s, matForward);

	//Rotation
	MAT44 matRotation;
	matRotation.identity();
	mat4FromQuat(r, matRotation);
	matForward = mat4Mult(matForward, matRotation);

	//Set Translate
	mat4Translate(t, matForward);

	//Compute Backward Matrix
	mat4Invert(matForward, matBackward);

	if(mat4IsIdentity(matForward))
		return 0;
	else
	{
		...
		{
			m_lpMtxNode->matrix[idxMtxElement + 0] = matBackward.e[0][0];
			m_lpMtxNode->matrix[idxMtxElement + 1] = matBackward.e[1][0];
			m_lpMtxNode->matrix[idxMtxElement + 2] = matBackward.e[2][0];
			m_lpMtxNode->matrix[idxMtxElement + 3] = matBackward.e[3][0];

			m_lpMtxNode->matrix[idxMtxElement + 4] = matBackward.e[0][1];
			m_lpMtxNode->matrix[idxMtxElement + 5] = matBackward.e[1][1];
			m_lpMtxNode->matrix[idxMtxElement + 6] = matBackward.e[2][1];
			m_lpMtxNode->matrix[idxMtxElement + 7] = matBackward.e[3][1];

			m_lpMtxNode->matrix[idxMtxElement + 8] = matBackward.e[0][2];
			m_lpMtxNode->matrix[idxMtxElement + 9] = matBackward.e[1][2];
			m_lpMtxNode->matrix[idxMtxElement + 10] = matBackward.e[2][2];
			m_lpMtxNode->matrix[idxMtxElement + 11] = matBackward.e[3][2];
		}
		m_lpMtxNode->count++;

		//Copy Forward Matrix for Box Manipulation
		const float* lpForward = matForward.cptr();
		idxMtxElement = idxMatrix * BOX_MATRIX_STRIDE;
		for(int i=0; i<BOX_MATRIX_STRIDE; i++)
			m_lpMtxBox->matrix[idxMtxElement + i] = lpForward[i];
		m_lpMtxBox->count++;
...
bool ExportMeshAsObj(const char* lpFilePath, const PolyMPUs& poly)
{
	...
	DAnsiStr strOut = DAnsiStr("#Exported from ParsipCmd\n");
	ofs << strOut;
	strOut = DAnsiStr("o cube1\n");
	ofs << strOut;
	...
	{
		v = svec3f(arrVertices[i * 3], arrVertices[i * 3 + 1], arrVertices[i * 3 + 2]);
	...
	for(U32 i=0; i<ctVertices; i++)
	{
		n = svec3f(arrNormals[i * 3], arrNormals[i * 3 + 1], arrNormals[i * 3 + 2]);
