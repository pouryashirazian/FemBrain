	...
	if(bCheckBoxes)
	{
		...
		for(U32 i=0; i < blobOps.count; i++)
		{
			...
			if(!bRange)
			{
				...
				if(bLeftChildIsOp)
				{
					child1BoxLo = svec3f(blobOps.bboxLoX[idxLC], blobOps.bboxLoY[idxLC], blobOps.bboxLoZ[idxLC]);
					child1BoxHi = svec3f(blobOps.bboxHiX[idxLC], blobOps.bboxHiY[idxLC], blobOps.bboxHiZ[idxLC]);
				}
				else
				{
					child1BoxLo = svec3f(blobPrims.bboxLoX[idxRC], blobPrims.bboxLoY[idxRC], blobPrims.bboxLoZ[idxRC]);
					child1BoxHi = svec3f(blobPrims.bboxHiX[idxRC], blobPrims.bboxHiY[idxRC], blobPrims.bboxHiZ[idxRC]);
				}

				if(bRightChildIsOp)
				{
					child2BoxLo = svec3f(blobOps.bboxLoX[idxLC], blobOps.bboxLoY[idxLC], blobOps.bboxLoZ[idxLC]);
					child2BoxHi = svec3f(blobOps.bboxHiX[idxLC], blobOps.bboxHiY[idxLC], blobOps.bboxHiZ[idxLC]);
				}
				else
				{
					child2BoxLo = svec3f(blobPrims.bboxLoX[idxRC], blobPrims.bboxLoY[idxRC], blobPrims.bboxLoZ[idxRC]);
					child2BoxHi = svec3f(blobPrims.bboxHiX[idxRC], blobPrims.bboxHiY[idxRC], blobPrims.bboxHiZ[idxRC]);
				}
					...
					SOABlobBoxMatrices& boxMatrices)
{
	...
	bboxLo = svec3f(FLT_MAX, FLT_MAX, FLT_MAX);
	bboxHi = svec3f(FLT_MIN, FLT_MIN, FLT_MIN);
	for(U32 i=0; i < blobPrims.count; i++)
	{
		svec3f curLo = svec3f(blobPrims.bboxLoX[i], blobPrims.bboxLoY[i], blobPrims.bboxLoZ[i]);
		svec3f curHi = svec3f(blobPrims.bboxHiX[i], blobPrims.bboxHiY[i], blobPrims.bboxHiZ[i]);
				  		 ...
				  		 SOABlobBoxMatrices& boxMatrices)
{
	...
	//Boxes are provided with input structure in future
	{
		...
		for(U32 i=0; i<blobPrims.count; i++)
		{
			svec3f curLo = svec3f(FLT_MAX, FLT_MAX, FLT_MAX);
			svec3f curHi = svec3f(FLT_MIN, FLT_MIN, FLT_MIN);
			svec3f seed = svec3f(blobPrims.posX[i], blobPrims.posY[i], blobPrims.posZ[i]);

			switch(blobPrims.type[i])
			{
			case(primPoint): case(primNULL):
				{
					svec3f off = svec3f(offset, offset, offset);
					svec3f pos = svec3f(blobPrims.posX[i], blobPrims.posY[i], blobPrims.posZ[i]);
					curLo = vsub3f(pos, off);
					curHi = vadd3f(pos, off);
				}
				break;
			case(primLine):
				{
					...
					svec3f expand = vadd3f(vscale3f(offset, svec3f(1.0f, 1.0f, 1.0f)), vscale3f(3.0f*offset, vsub3f(s1, s0)));
					curLo = vsub3f(s0, expand);
					curHi = vadd3f(s1, expand);
				}
				break;
			case(primRing):
				{
					...
					svec3f expand = vadd3f(vscale3f(radius + offset, dirComp), vscale3f(offset, dir));
					curLo = vsub3f(pos, expand);
					curHi = vadd3f(pos, expand);
				}
				break;
			case(primDisc):
				{
			...
			case(primCylinder):
				{
					...
					svec3f expand = vadd3f(vscale3f(offset + radius, svec3f(1.0f, 1.0f, 1.0f)), vscale3f(0.5f*offset, dir));
					curLo = vsub3f(s0, expand);
					curHi = vadd3f(s1, expand);
				}
				break;
			case(primCube):
				{
					...
					curLo = vsub3f(s0, svec3f(side, side, side));
					curHi = vadd3f(s0, svec3f(side, side, side));
				}
				break;
			case(primTriangle):
				{
					...
					curLo = vsub3f(vmin3f(vmin3f(s0, s1), s2), off);
					curHi = vadd3f(vmax3f(vmax3f(s0, s1), s2), off);
				}
				break;
			...
			if(idxMatrix != 0)
			{
				...
				//Set BBOX
				svec3f curLoT = mat4Transform(mat, curLo);
				svec3f curHiT = mat4Transform(mat, curHi);

				//Update Lo and Hi
				curLo = vmin3f(curLoT, curHiT);
				curHi = vmax3f(curLoT, curHiT);

				//Set SEED
				seed = mat4Transform(mat, seed);
			}
						  	...
						  	svec3f& boxLo, svec3f& boxHi)
{
	...
	if(bRange)
	{
		...
		{
			primBoxLo = svec3f(blobPrims.bboxLoX[i], blobPrims.bboxLoY[i], blobPrims.bboxLoZ[i]);
			primBoxHi = svec3f(blobPrims.bboxHiX[i], blobPrims.bboxHiY[i], blobPrims.bboxHiZ[i]);
			...
			{
				boxLo = vmin3f(boxLo, primBoxLo);
				boxHi = vmax3f(boxHi, primBoxHi);
			}
		}
	...
	{
		//Left Child Box
		svec3f boxLoLC, boxHiLC;
		if(bLeftChildOp)
			PrepareOpBBox_Recursive(idxLC, blobOps, blobPrims, boxMatrices, boxLoLC, boxHiLC);
		else
		{
			boxLoLC = svec3f(blobPrims.bboxLoX[idxLC], blobPrims.bboxLoY[idxLC], blobPrims.bboxLoZ[idxLC]);
			boxHiLC = svec3f(blobPrims.bboxHiX[idxLC], blobPrims.bboxHiY[idxLC], blobPrims.bboxHiZ[idxLC]);
		}
		...
		if(bUnary)
		{
			...
			if(idxMatrix != 0)
			{
				...
				//Set BBOX
				svec3f curLoT = mat4Transform(mat, boxLoLC);
				svec3f curHiT = mat4Transform(mat, boxHiLC);

				boxLo = vmin3f(curLoT, curHiT);
				boxHi = vmax3f(curLoT, curHiT);
		...
		{
			svec3f boxLoRC, boxHiRC;
			if(bRightChildOp)
				PrepareOpBBox_Recursive(idxRC, blobOps, blobPrims, boxMatrices, boxLoRC, boxHiRC);
			else
			{
				boxLoRC = svec3f(blobPrims.bboxLoX[idxRC], blobPrims.bboxLoY[idxRC], blobPrims.bboxLoZ[idxRC]);
				boxHiRC = svec3f(blobPrims.bboxHiX[idxRC], blobPrims.bboxHiY[idxRC], blobPrims.bboxHiZ[idxRC]);
			}

			boxLo = vmin3f(boxLoLC, boxLoRC);
			boxHi = vmax3f(boxHiLC, boxHiRC);
	...
	for(U32 i=0; i<blobPrims.count; i++)
	{
		if(blobPrims.type[i] == primInstance)
		{
			...
			if(isOriginOp)
			{
				...
				curLo = svec3f(blobOps.bboxLoX[idxOrigin], blobOps.bboxLoY[idxOrigin], blobOps.bboxLoZ[idxOrigin]);
				curHi = svec3f(blobOps.bboxHiX[idxOrigin], blobOps.bboxHiY[idxOrigin], blobOps.bboxHiZ[idxOrigin]);
			}
			else
			{
				assert(blobPrims.type[idxOrigin] == originNodeType);
				curLo = svec3f(blobPrims.bboxLoX[idxOrigin], blobPrims.bboxLoY[idxOrigin], blobPrims.bboxLoZ[idxOrigin]);
				curHi = svec3f(blobPrims.bboxHiX[idxOrigin], blobPrims.bboxHiY[idxOrigin], blobPrims.bboxHiZ[idxOrigin]);
			}
			...
			//Set SEED
			svec3f seed = vscale3f(0.5f, vadd3f(curLo, curHi));
			blobPrims.seedX[i] = seed.x;
			blobPrims.seedY[i] = seed.y;
			blobPrims.seedZ[i] = seed.z;
		}
			...
			for(int k=0; k<GRID_DIM-1; k++)
			{
				...
				for(int icase=0; icase<16; icase++)
				{
					...
					if(idxMeshVertex[icase] == -1)
					{
						...
						e1 = svec3f(arrPosX[interval - 1], arrPosY[interval - 1], arrPosZ[interval - 1]);
						e2 = svec3f(arrPosX[interval], arrPosY[interval], arrPosZ[interval]);
...
/*
int CMPUProcessor::computeRootNewtonRaphson(const svec3f& p1, const svec3f& p2,
											float fp1, float fp2, 
										  ...
										  Float_& primField, U32 idxPrimitive) const
{
	...
	{
		/*
		svec3f c = svec3f(m_blobPrims.posX[idxPrimitive],
						m_blobPrims.posY[idxPrimitive],
						m_blobPrims.posZ[idxPrimitive]);

		svec3f n = svec3f(m_blobPrims.dirX[idxPrimitive],
		...
		for(int i=0;i<PS_SIMD_FLEN; i++)
		{
			...
			svec3f pn = svec3f(ptX[i], ptY[i], ptZ[i]);
			svec3f pnc = vsub3f(pn, c);

			svec3f dir =  vsub3f(pnc, vscale3f(vdot3f(n, pnc), n));
			...
			else
			{
				...
				svec3f x = vadd3f(c, vscale3f(r, dir));
...
bool FieldComputer::computeRootSimd(const Float_& pX, const Float_& pY, const Float_& pZ, svec3f& outRoot) const
{
	outRoot = svec3f(0.0f, 0.0f, 0.0f);
	...
	//Update e1 and e2 for the high resolution field
	svec3f e1 = svec3f(pX[interval - 1], pY[interval - 1], pZ[interval - 1]);
	svec3f e2 = svec3f(pX[interval], pY[interval], pZ[interval]);

	float scale = (ISO_VALUE - F[interval - 1])/(F[interval] - F[interval-1]);
	outRoot = vadd3f(e1, vscale3f(scale, vsub3f(e2, e1)));
	return true;
}
