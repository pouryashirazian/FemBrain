...
void Cutting::setup() {
	...
	DAnsiStr strCuttingFP = strCodePath + DAnsiStr("PS_Shaders/Cutting.cl");

	//Compute Device
	m_lpDevice = new ComputeDevice(ComputeDevice::dtGPU, true, false, "AMD");
	ComputeProgram* lpProgram = m_lpDevice->tryLoadBinaryThenCompile(strCuttingFP.cptr());

	//Setup Kernels
	m_lpKernelFacePoints = lpProgram->addKernel("ComputePerTetFaceIntersections");
	m_lpDevice->getKernelWorkgroupSize(m_lpKernelFacePoints);

	m_lpKernelEdgePoints = lpProgram->addKernel("ComputePerTetEdgeIntersections");
	m_lpDevice->getKernelWorkgroupSize(m_lpKernelEdgePoints);

	m_lpKernelCentroids = lpProgram->addKernel("ComputePerTetCentroids");
	m_lpDevice->getKernelWorkgroupSize(m_lpKernelCentroids);

	m_lpKernelTest = lpProgram->addKernel("ComputeSegmentTriIntersections");
	m_lpDevice->getKernelWorkgroupSize(m_lpKernelTest);
...
bool Cutting::createMemBuffers() {
	...
	//Buffers
	m_inMemTetMeshVertices = m_lpDevice->createMemBuffer(szVertexBuffer, ComputeDevice::memReadOnly);
	m_inMemTetMeshIndices = m_lpDevice->createMemBuffer(szElementsBuffer, ComputeDevice::memReadOnly);
	m_inMemMeshInfo = m_lpDevice->createMemBuffer(sizeof(MeshInfo), ComputeDevice::memReadOnly);

	//Face
	m_inoutMemFaceFlags = m_lpDevice->createMemBuffer(sizeof(U32) * m_meshInfo.ctTets * 4, ComputeDevice::memReadWrite);
	m_inoutMemFacePoints = m_lpDevice->createMemBuffer(szFacePointsBuffer, ComputeDevice::memReadWrite);

	//Edge
	m_inoutMemEdgeFlags = m_lpDevice->createMemBuffer(sizeof(U32) * m_meshInfo.ctTets * 6, ComputeDevice::memReadWrite);
	m_inoutMemEdgePoints = m_lpDevice->createMemBuffer(szEdgePointsBuffer, ComputeDevice::memReadWrite);
...
int Cutting::computeFaceIntersections(const vec3d& s0, const vec3d& s1) {
	...
	cl_mem inMemScalpelEdge = m_lpDevice->createMemBuffer(sizeof(float) * 8, ComputeDevice::memReadOnly);
...
__kernel void ComputePerTetFaceIntersections(__global float4* arrInVertices,
	 ...
	 */
	//FacePoints Buffer
	cl_mem glMemFacePoints = m_lpDevice->createMemBufferFromGL(m_lpDrawFacePoints->handle(), ComputeDevice::memReadWrite);
...
int Cutting::computeFaceCentroids() {
	...
	*/
	...
	//FacePoints Buffer
	cl_mem outMemFacePoints = m_lpDevice->createMemBufferFromGL(m_lpDrawFacePoints->handle(), ComputeDevice::memReadWrite);
...
int Cutting::computeFaceSegmentIntersectionTest() {
	...
	cl_mem inMemVertices = m_lpDevice->createMemBuffer(sizeof(float) * 12, ComputeDevice::memReadOnly);
	...
	minfo.ctTets = 1;
	minfo.ctVertices = 3;
	cl_mem inMemMeshInfo = m_lpDevice->createMemBuffer(sizeof(MeshInfo), ComputeDevice::memReadOnly);
	if(!m_lpDevice->enqueueWriteBuffer(inMemMeshInfo, sizeof(MeshInfo), &minfo))
		return -1;

	//FacePoints
	cl_mem inoutMemFacePoints = m_lpDevice->createMemBuffer(sizeof(float) * 4 * minfo.ctTets, ComputeDevice::memReadWrite);
...
int Cutting::computeEdgeIntersections(const vec3d sweptQuad[4]) {
	...
	cl_mem inMemSweptQuad = m_lpDevice->createMemBuffer(sizeof(float) * 16, ComputeDevice::memReadOnly);
...
int Cutting::performCut(const vec3d& edge0, const vec3d& edge1) {
	...
	if (m_vCuttingPath.size() > 1) {
		...
		for (int i = (int) m_vCuttingPath.size() - 1; i >= 0; i--) {
			double d = vec3d::distance(m_vCuttingPath[i].first, edge0);
	...
	m_vCuttingPath.push_back(make_pair<vec3d>(edge0, edge1));
